...
AlloyTrainingSchema = DataFrameSchema(
    columns={
        ...
        # Compositions are stored as floats, between 0.0 and 1.0 (weight fractions)
        "Ti": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Nb": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Zr": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Ta": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Mo": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Fe": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Al": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "V": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Cr": Column(float, Check.in_range(0.0, 1.0), nullable=True),
        "Ni": Column(float, Check.in_range(0.0, 1.0), nullable=True),
    ...
    # Ensure columns representing weights sum to roughly 1.0
    checks=[
        Check(
            lambda df: (df[["Ti", "Nb", "Zr", "Ta", "Mo", "Fe", "Al", "V", "Cr", "Ni"]].fillna(0.0).sum(axis=1) - 1.0).abs() < 1e-2,
            name="check_wt_percent_sum_1"
        )
    ],
    coerce=True
)
